 ...
 * 功能分区：
 * - 弹窗工具：提示 / 确认
 * - 数据层：版本加载、设置读取与保存（invoke + settings-changed 事件）
 * - 主题：内置与用户主题卡片渲染、应用、删除
 * - 下拉框框架：portal 定位（window.settings_init_all_selects 供开发者选项复用）
 * - 各页绑定：语言 / Canvas / 文档阅读器 / 存储 / 关于 / 更新
 * - 极光背景动画、开发者选项入口
 */
 ...
 */
export async function init_settings_panel() {
...
async function initSettings() {
    ...
    /**
     * 同步"默认粗细"指示点（画笔/橡皮擦）
     * @param {string} group - 'pen' | 'eraser'
     */
    function settings_update_default_dots(group) {
        const saved = group === 'pen'
            ? (_loadedSettings.penWidth !== undefined ? _loadedSettings.penWidth : 5)
            : (_loadedSettings.eraserSize !== undefined ? _loadedSettings.eraserSize : 15);
    ...
    function settings_read_presets_from_ui() {
        ...
        for (let i = 0; i < 5; i++) {
            const input = document.getElementById('penPreset' + i);
            values.push(input ? parseInt(input.value) || DEFAULT_PRESETS[i] : DEFAULT_PRESETS[i]);
        }
        return values;
    }
    for (let i = 0; i < 5; i++) {
    ...
    function settings_read_eraser_presets_from_ui() {
        ...
        for (let i = 0; i < 5; i++) {
            const input = document.getElementById('eraserPreset' + i);
            values.push(input ? parseInt(input.value) || DEFAULT_ERASER_PRESETS[i] : DEFAULT_ERASER_PRESETS[i]);
        }
        return values;
    }
    for (let i = 0; i < 5; i++) {
    ...
    if (eraserPresetRestore) {
        eraserPresetRestore.addEventListener('click', async () => {
            const defaults = DEFAULT_ERASER_PRESETS;
            for (let i = 0; i < 5; i++) {
                const input = document.getElementById('eraserPreset' + i);
                if (input) input.value = defaults[i];
            }
