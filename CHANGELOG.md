...
- **惯性滑动（阅读器 / 小黑板拖拽平移）**：move 模式拖拽平移松手后，按手势末速度继续滑行并指数减速自然停住（猛甩约滑行 1 秒、轻抛约 0.6 秒），滑行中撞到可移动边界即停。基础设施（速度估计 / 惯性 tick / 边界处理）首提交起就存在但一直被 `momentumEnabled: false` 关闭、小黑板入口被掏空，本次启用并重做了手感的两个根子：
  - **速度估计按时间归一化**：原先按「每次输入事件的位移」估计速度，事件频率不同（鼠标轮询率 / 触屏 60Hz / 手写笔）会得到完全不同的甩动距离；现折算为「每 16.7ms 的位移」再做 EMA 平滑，三种输入手感一致。松手前停顿 >120ms 视为无意甩动，不触发惯性。
  - **减速与位移帧率无关**：原先摩擦系数按帧写死（0.85~0.65/帧，衰减极快且 120Hz 屏上滑行距离减半）；现改为 `v *= 0.94^(dt/16.7)` 的指数衰减 + 位移按 dt 缩放，任何刷新率下滑行轨迹一致。滑行结束仍走原有空闲结算（`_dr_apply_scale` + 页面可见性检查 / 黑板平滑落定）。

### 新增

- **恢复上次文档细分设置**：原「恢复上次文档状态」总开关拆分为两项——
  - **笔迹保存**：保存 / 不保存 / **每次关闭时询问**（三选一）。询问模式弹出确认对话框（保存并关闭 / 不保存关闭）；Rust 侧新增 `app_close_dialog_mode` 命令，弹窗期间 6 秒保存兜底让位、改用 10 分钟长兜底（webview 冻结或用户长期不响应时仍能强制退出，不会永远关不掉）。
  - **浏览记录**（页码 / 缩放 / 位置）：纯开关，不参与关闭询问。
  - 退出路径按细分开关分别处理：不保存笔迹时**清空批注缓存文件**（防止旧笔迹在「不保存」状态下复活）；两者全关时清理 lastOpenDoc。beforeunload 兜底同策略，询问模式下兜底不擅自替用户写盘。
  - 旧配置键迁移链：`restoreStrokesMode` → `restoreSaveStrokes`/`restoreAskOnClose` → `restoreLastDoc`，逐级回退保证升级后行为一致。
- **LoAF 长帧监视器**（`src/modules/perf/loaf-monitor.js`，纯诊断零依赖）：long-animation-frame 五相耗时归因（script / blocking / styleLayout / paint 按时间戳推导）、GPU renderer 懒探测（排除软件光栅）、longtask 并行观察、堆采样、rAF 帧间隔监视（>200ms 空洞计数并上控制台自报）。为真机「主线程空闲但合成器/GPU 侧停摆」类帧饿提供观测基线。
- **PDF 栅格化卸载到独立渲染线程**（`render-worker`）：pdf.js 文档字节在 worker 侧加载（按文档身份记忆零重复拷贝），并发渲染 + 优先级派发（可见页/占位队首、预渲染/缩略图队尾）；主线程只做参数守卫与画布换帧，滚动/缩放期间栅格化不再阻塞主线程。任何失败回退主线程 pdf.js 路径，渲染线程是纯加速层不承担正确性。
- **批注缓存序列化移入维护线程**（`maintenance-worker`）：大笔画文档（数 MB）的 JSON.stringify / parse 在独立线程执行，保存 stringify 不卡结算帧、恢复 parse 不阻塞打开流程；与渲染线程刻意分离，序列化不挤占栅格化并发槽位；失败回退主线程。
- **已渲染页位图缓存**（字节预算 LRU，128MB / 10 条）：页面虚拟化卸载前 `createImageBitmap` 留档，翻回该页直接回贴——「卸载后反复重新渲染」的根治点。同参数缓存位图直接回贴、低清旧位图先回贴占位防白屏；两轮驱逐先跳过可见页/活动页。

### 性能优化

- **手势期渲染暂停**：平移/惯性/滚轮/捏合的每帧续期 120ms 静默窗口，窗口内不启动新渲染、渲染线程限流；停稳后先回贴挂起提交、再排空瓦片队列、最后补扫可见性，零遗漏。
- **滑动低清占位**：手势期进入视口的未加载页立即按 0.5x 渲染并即时上屏（豁免门控，防整段手势期白屏），停稳后统一升清；紧急预渲染泵手势期改派占位形态 + 滚动前缘 DOM 预挂载。
- **挂起提交回贴 + 48ms 全局提交节流**（真机数据驱动）：全幅提交洪峰会打爆合成器上传管线（每条 commit 前后跟随 300~600ms rAF 帧饿而主线程全程空闲）；非占位全幅提交之间强制 ≥48ms 间距，挂起结果逐帧限量回贴（6ms 帧预算）。
- **渲染结果统一收尾 `_pdf_commit_render_result`**：主线程/渲染线程两条路径的收尾合并为单一方法；同尺寸提交不再重赋 `canvas.width/height`（重分配 = 合成器纹理重建，真机实测每条 ~200ms 帧饿与上传体积无关）。
- **批注瓦片延后初始化 + 空页懒加载**：手势期瓦片初始化入错峰队列（每帧一页真实初始化不变式保留）；**无笔迹页直接标记 deferred 零成本出队**，快滑穿过数十个无批注页后不再空耗几十帧，有批注页的瓦片更快就位；落笔前瓦片自愈兜底。
- **GPU 回收改增量分批执行**：旧实现每次隐藏定时器触发 O(N) 全量扫描 + 当场回收（快滑 40 页实测单次最大 57.7ms 主线程尖峰）；现扫描只收集入队（400ms 节流合并），每批 ≤3 页经 rIC 分批执行，手势期顺延、积压超限小批快泄。
- **页面栅格化 DPR 阶梯量化 + 降档滞回**：渲染请求向上取整到最近一档（0.5~8），档内缩放由 CSS transform 覆盖——连续缩放不再每个停顿都全量重栅格化；缩小时现有渲染高出目标一档以内（≤1.35x）保留不重渲染，抑制「放大↔缩小」振荡。
- **主画布/黑板包装器常驻 `will-change: transform`**：平移/缩放走纯合成器（与 Edge 滚动同机制），避免手势中 Chromium 重定光栅比例造成整层重栅化突发。
- **瓦片画布启用 GPU 光栅化**：去掉 `willReadFrequently` hint（会把画布强制回退成软件光栅，CPU 画完再整块上传纹理，是滚动/缩放期间 GPU 占用与上传带宽的结构性来源）；产品热路径没有「从瓦片读像素」需求。
- **虚拟化不再驱逐 PDFPage 解析缓存**（由 LRU 上限 12 统一管理）；预载窗口与缓存上限对齐（旧半径 10 共 21 页 > 上限 12，预载自己驱逐自己）；PDFPage LRU 命中重插修复（防退化为 FIFO）。

### 修复

- **快滑后模糊占位页要点一下才升清**：48ms 节流门挂起的升清渲染结果没有回贴调度——resume 的 flush 快照不含停稳补扫新增的结果，位图滞留挂起表直到下一次手势。现非手势期节流门挂起自动调度 rAF 回贴；停稳后模糊页按 ~48ms/页渐进升清。
- **缩放时移动有概率导致文档消失**：捏合越界松手的回弹 transform 交给 250ms CSS 过渡动画，可见性扫描发生在 t≈0（越界起点），落点处真正可见的页被判 hidden 并被虚拟化卸载，动画期间纯 CSS 插值永远等不来补扫。现回弹清理回调强制重测 + 补扫（250ms 补扫 < 800ms 回收延迟，来得及取消）。
- **可见性差集式 hidden**：可见性扫描只覆盖预渲染窗口——窗口外页 `is_visible` 卡 true 成僵尸页（逃过全部 GPU 清理，长文档快滑合成层内存暴涨），窗口内页又被每帧无条件 hidden（防抖定时器形同虚设）。现上帧可见、本帧不在可见集才 hidden，恰好一次。
- **预渲染去重与方向修复**：新增 `_prerender_already_adequate` 判据（渲染完成后承诺即清空，「无在途承诺」推不出「没渲染过」，否则泵每帧空转）；已虚拟化/已滚离页不预渲染（重建成果随即被卸载）；下滑方向判定此前与「1=向文档后方」约定相反，向已滚过的后方页预渲染再被清理（mount/unmount 循环燃料）。
- **绘画时 DPR 档位扩展至 6x 并加硬上限钳制**：叠加层 DPR 设置档新增 3x/4x/6x（1x 屏超采样让笔迹预览更细腻）；显式设置值钳制 [0.5, 6]，防导入异常大值把覆盖层画布像素尺寸放大到失控。

### 重构

- **小黑板加载管线重写**：`open()` 六阶段固定时序（串行化关闭 → 切板/历史隔离 → 滑入 → 几何对齐 → 按当前缩放建层 → 过渡与笔画加载并发），翻页加载令牌防并发交错；`_apply_viewport_geometry` 成为板面几何唯一写入者；瓦片网格按当前缩放创建（旧实现硬编码 1，非 1 缩放首建全错 DPR）；起笔前快进平滑变换（过渡期起笔会把偏移烘焙进笔画坐标，滚轮缩放后立即书写即整体错位）；删除像素级快照机制（`tile_snapshots`/`snapshot_*`），`stroke_history` 是唯一源数据；黑板渲染不再读写全局缩放。
